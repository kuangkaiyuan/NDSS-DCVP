...
\title{Invalidating the Analysis Knowledge, Partition Diversity for Code Virtualization Protection}
...
Code virtualized obfuscation, aka VM (Virtual Machine)-based obfuscation, has recently been used to protect software programs from unauthorized analyses \cite{cv,vmp,Themida,fang2011multi,wang2014tdvmp,wang2013nislvmp}. It protects programs by transforming their native binary code into bytecode for a self-defined instruction set architecture. At runtime, the execution instruction semantics of the original programs are fulfilled by a native interpreter bundled with the bytecode. VM-based obfuscation substantially raises the bar on reverse engineering a VM-obfuscated program, and is probably the most sophisticated protection method in the literature.

A general method for analyzing VM-obfuscated code first reverse analysis the virtual interpreter; and then uses this information to work out the semantics of individual bytecode instructions; and finally, recovers the original program's logic embedded in the bytecode program \cite{falliere2009inside,rolles2009unpacking}. In the above three-phase analysis, the second is the most time-consuming phase, which needs to analyze every bytecode instruction's \texttt{handler} (a procedure for interpreting the bytecode instruction) to get its semantics. The last phase can be efficiently completed with some automated analysis tools \cite{fightingoreans}. To further impede the analysis, some improvements have been proposed. For example, TDVMP \cite{wang2014tdvmp} obfuscates the \texttt{handlers} of bytecode instructions and enables the protected programs the ability of runtime diversity, which is effective for defeating dynamic analysis. Fang et al. \cite{fang2011multi} presented a multi-stage obfuscation method that iteratively transforms a program for many times using different interpretations. Although these approaches pose more obstacles to the analysis process, they fail to change the semantics of bytecode instructions. As a result, an analyst who is aware of the relationship between bytecode instructions and their semantics from previous analysis work does not bother to analyze the obfuscated \texttt{handlers} once again. Specifically, in this paper, we call the results of the semantics of bytecode instructions from previous analysis work the \textit{analysis knowledge}. With such \textit{knowledge}, the process of analyzing a VM-obfuscated program will proceed more efficiently and less toilsomely.

\begin{figure}[!t]
\centering
\includegraphics[width=1\columnwidth]{fig/figone.pdf}
\caption{The process of automated reverse analysis. There are four different target programs, A, B, C and D. In the first scenario, four target programs have the fixed semantic and structural features. It will allow an attacker builds a knowledge base by obtaining \textit{analysis knowledge} from one of them, and then realize the automated reverse analysis of other similar programs. In the second scenario, protection systems make them have different semantic and structural features. In this way, the attacker cannot realize the automated reverse analysis by using the knowledge base.}
\label{fig:one}
\end{figure}

Figure~\ref{fig:one} describes an attack scenario reusing the \textit{analysis knowledge} to achieve automated reverse analysis. There are four different programs to be protected, program A, B, C and D. In the first scenario, the protection system always follows a fixed semantic and structural features when protect four target program. It will allow an attacker to build an attack knowledge base by obtaining \textit{analysis knowledge} about the program A behavior, and then realize the automated reverse analysis of other similar programs. In the second scenario, protection systems provide different protection schemes for different programs, so that they have different semantic and structural features and the \textit{analysis knowledge} obtained from each program is not reusable. In this way, the attacker cannot realize the automated reverse analysis by using the matching of attack knowledge base, so he has to spend a lot of time to analyze every detail for each program.

To mitigate the effect of reuse of \textit{analysis knowledge}, we propose DCVP (Code Virtualization Protection with Diversity), a hardened code virtualized obfuscation system that obscures the semantics of bytecode instructions. The method we use is ISR (Instruction Set Randomization) \cite{kc2003countering}, which randomly changes the \textit{opcodes} of bytecode instructions and their semantics (as described in section~\ref{sec:random}). The randomization itself, however, is not sufficient to make a difference, since it is similar to monoalphabetic substitution encryption and is easy to be bypassed according to the non-uniform frequencies of bytecode instructions. Therefore, DCVP further partitions the bytecode program into several parts and the semantics of bytecode instructions in each part are randomized differently (as described in section~\ref{sec:partition-random}). As a result, the same bytecode instruction in different parts of the bytecode program probably has different semantics.

The contributions presented in this paper are summarized as follows:
\begin{enumerate}
  \item We present a detailed introduction to the internals of code virtualized obfuscation and point out that the existence of \textit{analysis knowledge} can help to reduce analyst's workload and speed up the analysis process.
  \item We propose a countermeasure to mitigate the effect of reuse of \textit{analysis knowledge}, which partitions the bytecode programs into serveral parts and randomizes the semantics of bytecode instructions in each part differently.
  \item We have implemented a POC prototype of our approach, namely DCVP, and conducted some evaluations in terms of its effectiveness as well as spatial and temporal overhead. The results show that DCVP is a promising and practical approach for mitigating the effect of reuse of \textit{analysis knowledge}.
\end{enumerate}
